...
class MJXDynamicsIdentifier:
    def run(self, config: MJXDynamicsIdentificationConfig) -> Path:
        ...
        mx_template = mjx.put_model(model)
        dx0 = mjx.put_data(model, initial_data)
        ...
        def metrics_fn(theta):
            ...
            q_mse = jnp.mean((sim_q - observed_q) ** 2)
            qdot_mse = jnp.mean((sim_qdot - observed_qdot) ** 2)
            ...
            return total, q_mse, qdot_mse, prior_penalty, values, sim_q, sim_qdot
        ...
        best_values = np.asarray(best_payload[4], dtype=float)
        best_sim_q = np.asarray(best_payload[5], dtype=float)
        best_sim_qdot = np.asarray(best_payload[6], dtype=float)
        history.append(
            {
                "iteration": 0,
                "loss": best_loss,
        ...
        for iteration in range(1, max(1, int(config.max_iterations)) + 1):
            ...
            current_values = np.asarray(payload[4], dtype=float)
            history.append(
                {
                    "iteration": iteration,
                    "loss": current_loss,
            ...
            if current_loss + 1e-9 < best_loss:
                ...
                best_sim_q = np.asarray(payload[5], dtype=float)
                best_sim_qdot = np.asarray(payload[6], dtype=float)
        ...
        save_json(
            {
                ...
                "trajectories": {
                    ...
                    "simulated": self._simulated_payload(trajectories, timestamps.tolist(), best_sim_q, best_sim_qdot),
                },
                "limitations": [
