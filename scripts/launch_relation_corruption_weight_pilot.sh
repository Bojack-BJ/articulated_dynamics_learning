...
variant=${1:?usage: $0 VARIANT GPU AXIS_WEIGHT LINE_WEIGHT}
gpu=${2:-0}
axis_weight=${3:-2.0}
line_weight=${4:-1.0}
extra_args=("${@:5}")
epochs=${EPOCHS:-5}
seed=${SEED:-20260831}
edge_positive_weight=${EDGE_POSITIVE_WEIGHT:-4.0}
...
slot=${SLOT_MODEL:-/lumos-vePFS/suzhou/Users/lixiaotong/articulated_relation_training/outputs/partnet_core_v1_training/cotracker_slots/motion_part_slots.pt}
initial="$workspace/outputs/neural_head_pivot_b_pilot_v1/robust_segment_relation_10ep_seed_20260831/slot_relation_head.pt"
out="$workspace/outputs/neural_head_pivot_b_pilot_v1/relation_corruption_${variant}_${epochs}ep_seed_${seed}"
...
CUDA_VISIBLE_DEVICES="$gpu" PYTHONPATH="$workspace/src" "$python" \
  ...
  --output-dir "$out" --epochs "$epochs" --object-batch-size 32 \
  --learning-rate 5e-5 --edge-positive-weight "$edge_positive_weight" \
  --axis-geometry-branch --axis-head-type vector_neuron \
  --vector-pivot-parameterization analytic_plane_residual_v1 \
  --joint-type-loss-weight 2.0 --axis-loss-weight "$axis_weight" \
  --axis-line-loss-weight "$line_weight" --slot-geometry-representation invariant_v1 \
  ...
  --trajectory-corruption-augmentation --trajectory-corruption-probability 0.7 \
  --trajectory-corruption-track-fraction 0.25 --trajectory-drift-scale-fraction 0.03 \
  --trajectory-spike-probability 0.15 --initial-relation-model "$initial" \
  --device cuda --seed "$seed" "${extra_args[@]}" >"$out/train.log" 2>&1
finished=$(date +%s)
printf '{"variant":"%s","edge_positive_weight":%s,"axis_weight":%s,"line_weight":%s,"started_unix":%s,"finished_unix":%s,"wall_seconds":%s}\n' \
  "$variant" "$edge_positive_weight" "$axis_weight" "$line_weight" "$started" "$finished" "$((finished-started))" >"$out/runtime.json"
